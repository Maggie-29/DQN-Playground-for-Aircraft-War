...
class DeepQNetwork:
    def __init__(
        ...
        self.epsilon = 0 if e_greedy_increment is not 0 else self.epsilon_max
        ...
        with tf.variable_scope('eval_net'):
            ...
            c_names, n_l1, w_initializer, b_initializer = \
                ...
                tf.random_normal_initializer(
                    0., 0.3), tf.constant_initializer(0.1)  # config of layers 第二个参数是神经元数量，原值是10
    ...
    def learn(self):
        ...
        if self.learn_step_counter % self.replace_target_iter == 0:
            ...
            self.print_msg('target_params_replaced')
    ...
    def plot_cost(self):
        x, y = np.arange(len(self.cost_his)), self.cost_his
        if len(x) == len(y):
            plt.plot(x, y)
            plt.ylabel('Cost')
            ...
            print("[RL] Plot cost error, please try again.")
    ...
    def save_model(self, path):
        ...
        print("[RL] Model saved as: " + path)
        return

    def load_model(self, path):
        saver = tf.train.Saver()
        saver.restore(self.sess, path)
        print("[RL] Model loaded: " + path)
        return

    def print_msg(self, content):
        print(time.strftime("%x %X: ") + content)
        return

    def set_params(self, r_d, e_g, r_t_i, m_s):
        self.gamma = r_d
        self.epsilon_max = e_g
